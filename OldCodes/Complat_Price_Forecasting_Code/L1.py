...
# Printing
########################################################################
def PostPrinting(TargetDate, SubmissionDate, StartAhead, Prediction):
...
## Generation Adding
#BeforeGen = PTAverageGen.copy()
#
#BeforeGen.rename(columns={'Demand':'AheadDemand',
#                          'prediction_date':'available_date2'}, inplace=True)
#
### Daily Average Price
#DailyDemand = pd.pivot_table(BeforeGen,values="AheadDemand",index=["Year","Month","Day"],aggfunc=np.mean)
#DailyDemand = pd.DataFrame(DailyDemand).reset_index()
#DailyDemand.rename(columns={'AheadDemand': 'AverageDemand'}, inplace=True)
#
#MergedDemand=pd.merge(BeforeGen,DailyDemand,how="left",on=["Year","Month","Day"])
#MergedDemand["available_date2"]=pd.to_datetime(MergedDemand["available_date2"])
#
#if StartAhead==2:
#    WeatherData["available_date2"]=pd.DatetimeIndex(WeatherData["available_date2"]).date + datetime.timedelta(days=1)
#
#DataUpgrade = pd.merge(WeatherData,MergedDemand[["available_date2","AheadDemand","AverageDemand"]],how="left",on="available_date2")
...
print DemandTestX[["available_date","prediction_date"]]
...
#Predict=pd.DataFrame(DemandTrainY)
#Predict["Predict"]=LoadPredict[0,0,:]
#Predict.to_csv('Compare.csv')
...
# WeatherPrice=pd.merge(WeatherPrice ,DemandSupport,how="left",on="prediction_date")
...
#Result = PostPrinting(TargetDate, SubmissionDate, StartAhead, SaveForecast2)
#Result.head()
