...
def OldWeatherRead(OldWeatherFile,WindSelection,WeatherRange):
    ...
    Single = OldWeather[ OldWeather.point==12]
    Single = Single[np.append(ID,WindSelection)]

    Twin = OldWeather[ OldWeather.point==13]
...
def SingleWeather(WindSelection,WeatherRange):
    ...
    for f in range(0,len(FileList)):
        ...
        Single = ReadData[ ReadData.point==12]
        Single = Single[np.append(TimeIndex,WindSelection)]

        Twin = ReadData[ ReadData.point==13]
...
PriceSelection =["WindSquareA","WindSquareC","WindSquareB","WindSquareD",
                 "temperature","precipitation","wind_gust","air_density","pressure",
                "wind_speed_100m","wind_speed"]
...
## Weather
TotalWeather_Load =TotalWeatherGen(LoadSelection,LoadRange,0)
TotalWeather_Wind =TotalWeatherGen(WindSelection,WindRange,0)
TotalWeather_Price=TotalWeatherGen(PriceSelection,PriceRange,0)
...
####### Wind Effect
    if WindEffect != 121:
        ...
        Price_Load = LoadForecasting2(KownWeather_Price,KnownPrice,"Price",0,"Oligar",'Price')
