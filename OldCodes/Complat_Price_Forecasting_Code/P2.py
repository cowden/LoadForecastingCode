...
##
WeatherPrice=pd.merge(Data,RawPrice,how="left",on="prediction_date")
...
#TotalTestX=TotalTestX[(TargetAhead[0] <= TotalTestX.Ahead) & (TotalTestX.Ahead <=TargetAhead[-1]) ]
TotalTestX["Hour"]=pd.DatetimeIndex(TotalTestX["prediction_date"]).hour
TotalTestX=TotalTestX.drop(["Year","Month","Day","Price","available_date","prediction_date"],1)
...
TotalTrainX=TotalTrainX.drop(["Year","Month","Day","Price","available_date","prediction_date"],1)
...
Weight=np.array([1])
BestGroup=np.array([0])
BestMachine=np.array([0])
...
Result = PostPrinting(TargetDateTime, SubmissionDate, StartAhead, SaveForecast2)
Result.head()
