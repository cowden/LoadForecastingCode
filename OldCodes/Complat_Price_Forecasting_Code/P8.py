...
def PriceReader(PriceFileLoc,PriceFileName):
    ...
    RawPrice["DayOnly"]=pd.to_datetime(RawPrice["DayOnly"])
...
#######################################################################
def OldWeatherPriceRead(OldPrice, OldWeatherLoc, OldWeatherFile):

    BeforePrice = OldPrice.copy()
    BeforePrice.rename(columns={'Price':'AheadPrice'}, inplace=True)
    BeforePrice["prediction_date"]=BeforePrice["prediction_date"] + datetime.timedelta(days=1)
    BeforePrice=BeforePrice[BeforePrice.Hour==23]
    BeforePrice["Year"]=pd.DatetimeIndex(BeforePrice["prediction_date"]).year
    ...
    ## Daily Average Price
    AveragePrice=pd.pivot_table(OldPrice,values="Price",index=["DayOnly"],aggfunc=np.mean)
    AveragePrice=pd.DataFrame(AveragePrice).reset_index()
    AveragePrice.rename(columns={'Price': 'AveragePrice'}, inplace=True)
    AveragePrice["DayOnly"]=AveragePrice["DayOnly"] + datetime.timedelta(days=1)
    AveragePrice["Year"]=pd.DatetimeIndex(AveragePrice["DayOnly"]).year
    ...
    AdditionalPrice=pd.merge(BeforePrice[["Year","Month","Day","AheadPrice"]],
                             AveragePrice[["Year","Month","Day","AveragePrice"]],on=["Year","Month","Day"])

    Price = pd.merge(OldPrice,AdditionalPrice,on=["Year","Month","Day"])
    del Price["DayOnly"]
    ...
    for t in range(2,19):
        ...
        SingleWeatherStation = pd.merge(SingleWeatherStation,Temp2,
                                        on=["prediction_date","Year","Month","Day","Hour"],suffixes=('',str(t)))

    SingleWeatherStation["Ahead"] = 1
    SingleWeatherStation["prediction_date"]=pd.to_datetime(SingleWeatherStation["prediction_date"])

    OldWeatherPrice = pd.merge(SingleWeatherStation,Price,
...
def SingleWeather(WeatherFileLoc):
    ...
    for f in range(0,len(FileList)):
        ...
        SingleWeatherStation=SingleWeatherStation[SingleWeatherStation["Ahead"]<6]
...
#######################################################################
def CrossVal(AvailableDate,TrainX,TrainY):
    ...
    for machindex in range(0,len(MachineList)):
        ...
        for gt in range(0,len(GroupDecision)):
            ...
            for train_index, test_index in kf:
                ...
                print CVTarget
                train_index=np.where(AvailableDate.available_date!=CVTarget)[0]
                test_index=np.where(AvailableDate.available_date==CVTarget)[0]
...
def LoadForecasting(WeatherData,PTAverageGen):
    ...
    WeatherDemand["WeekDay"]=pd.DatetimeIndex(WeatherDemand["prediction_date"]).weekday
    ...
    DemandTestX=DemandTestX.drop(["Year","Month","Day","Demand",
                                  "available_date",
                                  "prediction_date"],1)
    ...
    AvailableDate=DemandTrainX[["available_date","Ahead"]]
    ...
    DemandTrainX=DemandTrainX.drop(["Year","Month",
                                    "Day","Demand",
                                    "available_date",
                                    "prediction_date"],1)
...
#    Param['NGroup']=[2]
    ...
    ##
    #FunRMSE, LoadPredict=CrossVal(AvailableDate,DemandTrainX,DemandTrainY)
...
OldWeatherPrice=OldWeatherPriceRead(Price, OldWeatherLoc, "historical_weather.csv")
...
#WeatherData["WeekDay"]=pd.DatetimeIndex(WeatherData["prediction_date"]).weekday
...
##############          Energy
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
TempFileLoc="/Users/dueheelee/Documents/ComPlatt/Data/Forecast_PTDemand/"
Energy = pd.read_csv(TempFileLoc + "PTHourlyDemand.csv")  # PTHourlyDemand.csv  PDEnergy.csv
Energy["Date"]=pd.to_datetime(Energy["Date"])
Energy.rename(columns={'Date': 'prediction_date'},inplace=True)
...
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
WeatherPrice=pd.merge(WeatherData, Price,how="left",on=["prediction_date"])
...
WeatherPrice=pd.merge(WeatherPrice ,Energy[["prediction_date","Energy"]],how="left",on=["prediction_date"])


OldWeatherPrice2=pd.merge(OldWeatherPrice,PTAverageGen,on=["prediction_date","Year","Month","Day","Hour"])
OldWeatherPrice2=pd.merge(OldWeatherPrice2,Energy[["prediction_date","Energy"]],on=["prediction_date"])


# WeatherPrice=pd.concat([OldWeatherPrice2,WeatherPrice])
...
AvailableDate=TotalTrainX[["available_date","Ahead"]]
...
Answer=np.array([18.07,15,12.10,8.69,9,11.01,11.70,12.10,17.14,
                 17.36,17.46,16.15,13.48,9.20,7.59,6,6,7,9.85,19.75,25.01,24.32,20.9])
...
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
FunRMSE, SavePredict=CrossVal(AvailableDate,TotalTrainX,TotalTrainY)
...
MAE=np.absolute(SaveForecast[:23]-Answer).mean()
