...
def ParameterSetting():
    ...
    # Machine List
    Param['MachineList']=["GBM","RF"]   # LogReg KNN GBM 'LogReg', ,'Ridge' "Reg"
...
#    Param2['NGroup']=[1]
    ...
    ## Main Game Starts
    CrossVal(AvailableDate,DemandTrainX,DemandTrainY,Param2)
...
TotalWeather["DayOnly"]= pd.DatetimeIndex(TotalWeather["available_date"]).date
TotalWeather["DayOnly"]= pd.to_datetime(TotalWeather["DayOnly"])
...
# Answer
Answer=np.array([16,13.97,13.05,13.49,17.37,21.75,27.69,28.10,30,28.59,27.10,26.14,24.44,
                 22.57,21.86,21.94,21.39,20.43,22,25.29,28.69,26.14,23.69])



# New Parameter Setting
Param['MachineList']=["Reg","Ridge","GBM","RF","SVM"]
...
for i in range(0,len(UniqueNextGen)):
    ...
    if i==0:
        SavingAverage1=AverageForecast
    else:
        SavingAverage1=pd.concat([SavingAverage1,AverageForecast])
...
for i in range(0,len(UniqueNextGen)):
    ...
    if i==0:
        SavingAverage2=AverageForecast
    else:
        SavingAverage2=pd.concat([SavingAverage2,AverageForecast])
...
#FinalValue=SavingAverage.Forecast.values
