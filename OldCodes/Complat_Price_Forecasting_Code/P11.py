...
def ParameterSetting():
    ...
    Param={'CVN': 20, 'CVP':0.1, 'CVOptExternal':"HoldOut"}

    # Machine List
    Param['MachineList']=['Reg']   # LogReg KNN GBM 'LogReg', ,'Ridge' "Reg"
...
def CrossVal(AvailableDate,TrainX,TrainY,Param2):
    ...
    for cv in range(0,CVN):
        CVTarget1 = TargetDate - datetime.timedelta(days=(CVN+MaxAhead+1)) + datetime.timedelta(days=cv)
        CVTarget2 = CVTarget1 + datetime.timedelta(days=(MaxAhead-1))
        ...
        MachineValue=np.zeros((len(MachineList),MaxAhead*24))
        MachineSolution=np.zeros((len(MachineList),MaxAhead*24))
...
#    Param2['NGroup']=[1]
    ...
    UiqueNextGen=NextGenTest["prediction_date"].drop_duplicates()

    bp()
    for i in range(0,len(UiqueNextGen)):
        A=UiqueNextGen[i]
        Sub=NextGenTest[NextGenTest.prediction_date==A]
...
Param['MachineList']=["Ridge","RF","GBM","Reg"]
Param['GBM_NTry']=500      # 100 < x < 2000
Param['GBM_LRate']=0.1
Param['RF_NTree']=200
Param['NGroup']=[1]
Param['C']=1
