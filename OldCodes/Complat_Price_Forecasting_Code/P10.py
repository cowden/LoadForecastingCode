    ...
    for machindex in range(0,len(MachineList)):
        ...
        for gt in range(0,len(GroupDecision)):
            ...
            for train_index, test_index in kf:
                CVTarget = TargetDate - datetime.timedelta(days=(CVN)) + datetime.timedelta(days=cb)

                print CVTarget
                train_index=np.where(AvailableDate.PredictionDayOny!=CVTarget)[0]
                test_index=np.where(AvailableDate.PredictionDayOny==CVTarget)[0]
                ...
                for grindex in range(0,NG): # NG : Only Whatch the Day+1 min(5,NG)
                    ...
                    for i in range(0,MaxAhead):
                        SubForecast=Forecast[i*24:(i+1)*24]
                        SubTestY= TstY[i*24:(i+1)*24]
                        NormForecast=PostProcessing(SubForecast)
                        TempPerformance=PerMeasure(SubTestY,NormForecast)
                        print TempPerformance

                    Temp = Forecast.reshape(-1,24)
                    Temp2 = np.mean(Temp ,axis=0)
                    Temp3 = np.tile(Temp2,Temp.shape[0])

                    GroupResult[TestsGroup[grindex]]=Temp3
                    NormForecast=PostProcessing(Temp3)
                    TempPerformance=PerMeasure(TstY,NormForecast)
                    print Machine, "Main Group:", GD, "Sub Group:", grindex, "CV:  ",  TempPerformance
...
#    print "BestC: ", BestC, " Best Performance: ", BestPerformance
    ...
    Support=SVR(C=Param['C'], kernel='rbf', degree=3, epsilon=Param['Epsilon'],
                gamma=Param['Gamma'], shrinking=True,cache_size=200,verbose=0)
...
def LoadForecasting(WeatherData,PTAverageGen,DemandPT,Param2):
    ...
    AvailableDate["PredictionDayOny"]=pd.DatetimeIndex(AvailableDate["prediction_date"]).date
    AvailableDate["PredictionDayOny"]=pd.to_datetime(AvailableDate["PredictionDayOny"])
...
#    Param2['RF_NTree']=100
    ...
    NextGenTest["PredictionDayOny"]=pd.DatetimeIndex(NextGenTest["prediction_date"]).date
    NextGenTest["PredictionDayOny"]=pd.to_datetime(NextGenTest["PredictionDayOny"])

    for i in range(0,MaxAhead):
        A=TargetDate + datetime.timedelta(days=(i+1))
        Sub=NextGenTest[NextGenTest.PredictionDayOny==A]
...
TotalWeather = SingleWeather(WeatherFileLoc)
TotalWeather=pd.concat([OldWeather,TotalWeather])
...
# Available
AvailableDate=TotalTrainX[["prediction_date","DayOnly","Ahead"]]
AvailableDate["PredictionDayOny"]=pd.DatetimeIndex(AvailableDate["prediction_date"]).date
AvailableDate["PredictionDayOny"]=pd.to_datetime(AvailableDate["PredictionDayOny"])
...
# Answer
Answer=np.array([20.47,19.06,18.95,19.02,20,26,31.69,34.79,39.01,35.17,32.90,31.24,
                 31,29.97,29.20,26.14,25,23.79,26.71,29.95,31.49,30.25,28.09])




# New Parameter Setting
Param['MachineList']=["Reg","Ridge","RF","GBM"]
Param['GBM_NTry']=1000      # 100 < x < 2000
Param['GBM_LRate']=0.05
Param['RF_NTree']=100
Param['NGroup']=[1]
Param['C']=1
...
Tracker["PredictionDayOny"]=pd.DatetimeIndex(Tracker["prediction_date"]).date
Tracker["PredictionDayOny"]=pd.to_datetime(Tracker["PredictionDayOny"])

FinalValue=np.zeros(MaxAhead*24)
for i in range(0,MaxAhead):
    A=TargetDate + datetime.timedelta(days=(i+1))
    Sub=Tracker[Tracker.PredictionDayOny==A]
