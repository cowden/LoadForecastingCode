...
def CrossVal(Param,TrainX,TrainY):
    ...
    ## Cross Validation
    kf = KFold(shuffle=False, n_splits=Param['CVN'] )

    ## Play the Game
    SavePredict=np.zeros( ( len(MachineList) , len(TrainY) ) )
    CVRMSE=np.zeros( (len(MachineList), Param['CVN']) )
    MachineRMSE=np.zeros( len(MachineList) )
    for machindex in range(0,len(MachineList)):
        ...
        for train_index, test_index in kf.split(TrainX):
            ...
            CVRMSE[machindex,cc]=TempPerformance
            cc=cc+1
            print Machine, "CV:", cc, " MSE:",TempPerformance

        SingleMachineOutput=SavePredict[machindex, :]
        NormForecast=PostProcessing(SingleMachineOutput,Param)
        TempPerformance=PerMeasure(TrainY,NormForecast)
        MachineRMSE[machindex]=TempPerformance
        print Machine, TempPerformance

    TempResult=np.mean(SavePredict,axis=0)
    NormForecast=PostProcessing(TempResult,Param)
    print CVRMSE, PerMeasure(TrainY,NormForecast)
    ...
    return MachineRMSE, SavePredict
...
########################################################################
def GradientBoosting(Param,TrainX,TrainY):
    ...
    ParamGrid={'n_estimators': Param['GBM_NTry'],
               'learning_rate': Param['GBM_LRate'],
               'subsample': Param['GBM_SubSample'],
               'max_depth': Param['MaxDepth'],
               'min_samples_split': Param['GBM_Split'],
               'min_samples_leaf':Param['GBM_Leaf']}

    return GBM, ParamGrid
...
def RandomForest(Param,TrainX,TrainY):
    ...
    ## Grid Seaerch
    ParamGrid={'n_estimators': Param['RF_NTree'],'max_features':Param['RF_Feature'],
               'min_samples_split': Param['RF_Split'],'min_samples_leaf':Param['RF_Leaf']}
...
def Machinery(Param,MachineInfo,TrainX,TrainY,TestsX,TestsY):
    FunctionList={'Ridge': RidgeReg,
                  'RF': RandomForest,
                  'GBM': GradientBoosting,
                  }
    Parameters={'Param':Param,'TrainX':TrainX,'TrainY':TrainY}
    Estimator, GridParam=FunctionList[MachineInfo](**Parameters)
...
#		Attribute=KNearestNeighbors(TrainX,TrainY)
    ...
    CV=GridSearchCV(estimator=Estimator, n_jobs=-1, param_grid=GridParam, cv=Param['SubCVN'])
    CV.fit(TrainX,TrainY)
    print(CV.best_params_)

    TestOutput=CV.predict(TestsX)
